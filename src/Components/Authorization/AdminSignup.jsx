...
import { confirmPassword, validateEmail, validateMobile, validateName, validatePassword } from '../../utils/Authentication';
...
const AdminSignup = () => {
  ...
  const roles = ["Super Admin","Placement Officer","Coordinator"]
      ...
      <div className='container'>
        <div className='row justify-content-center'>
          <div className='col-lg-7 col-xl-8'>
            <Top show={show} setShow={setShow} title="Register Status" msg={`Resister Successfully ${form.fullName}`}/>
            <div className='card bg-secondary border-0 shadow-lg rounded-4'>
              <div className='card-body p-4 p-md-5'>
                <div className='text-center mb-4'>
                  <div className='bg-primary bg-opacity-25 rounded-circle d-inline-flex justify-content-center align-items-center mb-3' style={{ width: 72, height: 72 }}>
                    <i className='bi bi-shield-lock-fill fs-2 text-primary'></i>
                  </div>
                  <h2 className='fw-bold text-light'>Admin Registration</h2>
                  <p className='text-light opacity-75'>Create an administrator account</p>
                </div>

                <form onSubmit={handleSubmit}>
                  <div className='row g-3'>
                    <Input form={form.fullName} errors={errors.name} title={"Full Name"} danger={true} handleChange = {handleChange} name={"fullName"} placeholder="Enter Full name"/>
                    <Input form={form.username} errors={null} title={"Username"} danger={false} handleChange = {handleChange} name={"username"} placeholder="Enter User name"/>
                    ...
                    <Input form={form.employeeId} errors={null} title={"Employee ID"} danger={false} handleChange = {handleChange} name={"employeeId"} placeholder="Enter Employee ID"/>
                  
                    <OptionInpu title="Role" value={form.role} onChange={handleChange} Data={roles} msg={"Select Role"}/>
                   ...
                   <PassInput form={form.password} errors={errors.password} title={"Password "} danger={true} handleChange = {handleChange} name={"password"} showPassword={showPassword} setShowPassword={setShowPassword}/>
                    ...
                    <div className='col-12'>
                      <button className='btn btn-primary w-100 py-2 fw-semibold'>
                        <i className='bi bi-person-plus-fill me-2'></i>Create Admin Account
                      </button>
                    </div>
